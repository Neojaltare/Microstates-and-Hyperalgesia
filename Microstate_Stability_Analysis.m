...
%% Microstate Stability Analysis for Resting-State EEG
%
% This script performs repeated microstate analysis on resting-state EEG data 
% across multiple iterations to evaluate the stability of microstate solutions.
%
% The pipeline includes:
% - Selecting participants based on experimental group and exclusion criteria
% - Loading RS1 and RS2 EEG datasets (cleaned) into EEGLAB
% - Extracting GFP peaks and segmenting microstates using a modified K-means algorithm
% - Repeating clustering and backfitting procedures over 5 iterations to assess solution stability
% - Calculating microstate statistics (e.g., GEV, duration, occurrence, coverage, GFP)
% - Saving segmented data and summary statistics for each iteration
% - Plotting and saving topographic maps of identified microstate prototypes
%
% Notes:
% - Paths must be updated to reflect local directory structure
% - This script uses EEGLAB and the Microstate EEGLAB Toolbox
% - The microstate extraction uses 1000 GFP peaks and varies the number of states (3–6)
% - Results for each iteration are saved to a dedicated folder for downstream analysis
%
% Author: Ketan Jaltare
% Contact: ketan.jaltare@kuleuven.be
...
addpath(genpath('/Path to Matlab Code'))
...
for thisrep = 1:5
    ...
    writepath = char(strcat(EEGdir,"Microstates/",Sample,"/StabilityAnalysis/Together",num2str(thisrep)));
    status = mkdir(writepath);
    ...
    EEG = pop_saveset( EEG, 'filename',char(strcat(Sample,"MicroProtoTogether")),'filepath',writepath);
    ...
    for i = 1:numsubs
        ...
        if contains(EEG.filename,"RS1","IgnoreCase",true)
            EEG = pop_saveset( EEG, 'filename',char(strcat("RS1",Sample,"Subject",string(MicrostateData.Subject(i)))),'filepath',writepath);
        elseif contains(EEG.filename,"RS2","IgnoreCase",true)
            EEG = pop_saveset( EEG, 'filename',char(strcat("RS2",Sample,"Subject",string(MicrostateData.Subject(i)))),'filepath',writepath);
        end
    ...
    writetable(Microstats, strcat(writepath,'/','Microstats.xlsx'))
    ...
    %
    %     addpath('/Path to EEGLAB/eeglab2023.0')
    %     addpath(genpath('/Path to Matlab Code folder'))
    %
    %     % start EEGLAB to load all dependent paths
    %     eeglab nogui
    %     Path = '/Volumes/KetanData2/EMKJ/Exp3EEG/Microstates/Control/Together/';
    ...
    for i = 1:5
        ...
        saveas(gcf,strcat(writepath,'/Microstate',num2str(i),'.jpg'))
    end
